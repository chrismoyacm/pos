...
SELECT
    ...
    COALESCE(CAST(cr.LIMITE_CREDITO AS DECIMAL(12,2)), 1000.00) AS LIMITE_CREDITO,
    COALESCE(CAST(cr.SALDO_ACTUAL AS DECIMAL(12,2)), 0.00) AS SALDO_INICIAL,
    COALESCE((
        SELECT SUM(CAST(vt.TOTAL_CREDITO AS DECIMAL(12,2)))
        FROM VENTATICKETS vt
        WHERE vt.CLIENTE_ID = CONCAT('c-', LPAD(CAST(c.ID AS UNSIGNED), 3, '0'))
    ), 0.00) AS CREDITO_VENTAS,
    COALESCE((
        SELECT SUM(CAST(m.MONTO AS DECIMAL(12,2)))
        FROM MOVIMIENTOS m
...
ORDER BY (COALESCE(CAST(cr.SALDO_ACTUAL AS DECIMAL(12,2)), 0.00)
       + COALESCE((SELECT SUM(CAST(vt.TOTAL_CREDITO AS DECIMAL(12,2))) FROM VENTATICKETS vt WHERE vt.CLIENTE_ID = CONCAT('c-', LPAD(CAST(c.ID AS UNSIGNED), 3, '0'))), 0.00)
...
SELECT COALESCE(SUM(GREATEST(
    COALESCE(CAST(cr.SALDO_ACTUAL AS DECIMAL(12,2)), 0.00)
  + COALESCE((SELECT SUM(CAST(vt.TOTAL_CREDITO AS DECIMAL(12,2))) FROM VENTATICKETS vt WHERE vt.CLIENTE_ID = CONCAT('c-', LPAD(CAST(c.ID AS UNSIGNED), 3, '0'))), 0.00)
  - COALESCE((SELECT SUM(CAST(m.MONTO AS DECIMAL(12,2))) FROM MOVIMIENTOS m WHERE m.CLIENTE_ID = CONCAT('c-', LPAD(CAST(c.ID AS UNSIGNED), 3, '0')) AND m.TIPO LIKE 'credit_payment%'), 0.00)
